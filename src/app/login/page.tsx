...
function LoginFormContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirection handled inside login() / useEffect
    } catch (err: unknown) {
  ...
  return (
    <div className="flex min-h-[calc(100vh-12rem)] items-center justify-center py-10 px-4 sm:px-6">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="rounded-xl border border-indigo-100 bg-indigo-50/70 p-4 backdrop-blur-xs transition-all">
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div className="space-y-1 text-xs">
                ...
                <div className="text-indigo-700 space-y-0.5">
                  <p>Username: <code className="font-mono font-bold bg-white/80 px-1.5 py-0.5 rounded text-indigo-900">emilys</code></p>
                  <p>Password: <code className="font-mono font-bold bg-white/80 px-1.5 py-0.5 rounded text-indigo-900">emilyspass</code></p>
                </div>
              </div>
            </div>
            <button
              ...
              className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-indigo-600 px-2.5 py-1.5 text-xs font-semibold text-white shadow-xs hover:bg-indigo-700 active:scale-95 transition-all"
            >
        ...
        <div className="glass-card rounded-2xl p-6 sm:p-8 shadow-card">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-300 bg-white pl-10 pr-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 shadow-2xs focus:border-indigo-500 focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20 transition-all"
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full rounded-xl border border-slate-300 bg-white pl-10 pr-10 py-2.5 text-sm text-slate-900 placeholder-slate-400 shadow-2xs focus:border-indigo-500 focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20 transition-all"
                  disabled={isSubmitting}
                />
                <button
                  type="button"
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 px-4 py-3 text-sm font-semibold text-white shadow-md shadow-indigo-600/25 hover:from-indigo-700 hover:to-violet-700 focus:outline-hidden focus:ring-2 focus:ring-indigo-500/30 active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed transition-all"
            >
              {isSubmitting ? (
                <>
